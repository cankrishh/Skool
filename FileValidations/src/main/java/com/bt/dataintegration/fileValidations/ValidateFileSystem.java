...
public class ValidateFileSystem {
	...
	public static List<Path> seprateFiles(String basePath, String pattern,Path tempdir,Path rawDir)
			throws IOException {
		List<Path> inputhPaths = new ArrayList<Path>();
		List<String> decompressedFiles = new ArrayList<String>();
		Configuration conf = new Configuration();

		FileSystem fs = FileSystem.get(new Path(basePath).toUri(), conf);
		//List all files that matches the pattern
		FileStatus[] listStatus = fs.globStatus(new Path(basePath + pattern));

		// send all matched files to raw directory
		for (FileStatus fstat : listStatus) {
			fs.rename(fstat.getPath(), rawDir);
			

		}
		
		FileStatus[] listfile = fs.listStatus(rawDir);
		// list of all compressed files in raw directory
		for (FileStatus listStatus2 : listfile) {
			if(listStatus2.isFile() && (listStatus2.getPath().getName().toString().endsWith(".gz")|| listStatus2.getPath().getName().toString().endsWith(".lzo")
				...
				String fileName = listStatus2.getPath().getName().toString();
				String decompressedFileName = fileName.substring(0, fileName.lastIndexOf("."));
				decompressedFiles.add(decompressedFileName);		
			}
					
		}
		
		// move all decompressed file and copy all non compressed files to temp
		FileStatus[] finalListofFile = fs.listStatus(rawDir);
		for(FileStatus listoffile:finalListofFile){
			String fileName = listoffile.getPath().getName().toString();
			if(listoffile.isFile() && (listoffile.getPath().getName().toString().endsWith(".gz")|| listoffile.getPath().getName().toString().endsWith(".lzo")
					||listoffile.getPath().getName().toString().endsWith(".bz2")||listoffile.getPath().getName().toString().endsWith(".snappy"))){
				
			}
			else{
				if((listoffile.isFile()) && (decompressedFiles.contains(fileName))){
					fs.rename(listoffile.getPath(), tempdir);
				}
				else {
					if(listoffile.isFile()){
						FileUtil.copy(fs, listoffile, fs, tempdir, false, false, conf);
					}
				}
			}
		}
		FileStatus[] tempFiles = fs.listStatus(tempdir);
		for(FileStatus tempFile:tempFiles){
			inputhPaths.add(tempFile.getPath());
		}
	...
	public static void main(String[] args) throws Exception {
		...
		String basePath="hdfs://quickstart.cloudera:8020/user/HAASDEMO_01/file_validations/";
		String tempdir="hdfs://quickstart.cloudera:8020/user/HAASDEMO_01/temp";
		String rejectedDir="hdfs://quickstart.cloudera:8020/user/HAASDEMO_01/rejected";
		String successfileName="cust_success.txt";
		String lineStartsFrom = "3";
		String isTrailerPresent = "true";
		String rawDir = "hdfs://quickstart.cloudera:8020/user/HAASDEMO_01/raw";*/
		...
		Path rawDirPath=new Path(rawDir);
		Configuration conf=new Configuration();
		...
		for(FileStatus list:listStatus){
			...
			fs.delete(list.getPath());
			}
		}
		
		if(successFileFlag){
		rejectZeroSizeFile(basePath, fileTypeMask,rejectedDirPath);
		List<Path> filelist = seprateFiles(basePath,fileTypeMask,tempdirpath,rawDirPath);
